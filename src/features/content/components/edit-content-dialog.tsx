...
import { updateContentAction } from "../actions/content.actions";
...
export function EditContentDialog({ content, open, onOpenChange, isLocked = false }: EditContentDialogProps) {
  ...
  useEffect(() => {
    if (content) {
      form.reset({
        ...
        transition: content.transition as any,
        transitionDuration: (content as any).transitionDuration || 1.0,
  ...
  const onSubmit = async (values: CreateContentSchema) => {
    ...
    setIsPending(true);
    try {
      const res = await updateContentAction(content.id, values);
      if (res.success) {
      ...
      <DialogContent className="sm:max-w-none w-screen h-screen max-h-screen fixed inset-0 translate-x-0 translate-y-0 rounded-none border-none p-0 flex flex-col bg-background/95 backdrop-blur-2xl overflow-hidden z-50">
        ...
        <div className="h-16 px-6 sm:px-8 border-b border-border/60 flex items-center justify-between bg-card/90 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-black tracking-tight">Editar Parámetros de Contenido</DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground">
                Ajusta la multimedia, tiempos y transiciones de Framer Motion.
              </DialogDescription>
            </div>
          </div>
